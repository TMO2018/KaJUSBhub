...
void* lastINTransmission_k1;
void* lastOUTTransmission_k1;
BOOL Keyboard_out;
...
extern volatile unsigned char hid_report_in_k1[8];
extern volatile unsigned char hid_report_out_k1[1];
extern volatile CTRL_TRF_SETUP SetupPkt;
extern const BYTE configDescriptor1[];
extern volatile BYTE CtrlTrfData[8];


extern const struct{BYTE report[63];}hid_rpt01_k1;
...
{
keyboard[key].state = 0;
keyboard[key].debounceCount = 0;

rawKeyboardState[key] = 0;
}
...
if (column == 0) TRISD0 = 0; else TRISD0 = 1;
if (column == 1) TRISD1 = 0; else TRISD1 = 1;
if (column == 2) TRISD2 = 0; else TRISD2 = 1;
if (column == 3) TRISD3 = 0; else TRISD3 = 1;
if (column == 4) TRISD4 = 0; else TRISD4 = 1;
if (column == 5) TRISD5 = 0; else TRISD5 = 1;
if (column == 6) TRISD6 = 0; else TRISD6 = 1;
if (column == 7) TRISD7 = 0; else TRISD7 = 1;
...
if (input == 0) rawKeyboardState[currentKeyNumber] = 1;
else rawKeyboardState[currentKeyNumber] = 0;
}
}


if (RC2 == 0) rawKeyboardState[64] = 1; else rawKeyboardState[64] = 0;
}
...
{
currentKeyNumber = (column * 8) + row;
if (rawKeyboardState[currentKeyNumber] == 1 && rawKeyboardState[currentKeyNumber + 1] == 1)
adjacentRowCount++;
}
...
{
currentKeyNumber = (column * 8) + row;
rawKeyboardState[currentKeyNumber] = 0;
rawKeyboardState[currentKeyNumber + 1] = 0;
}
}
}
}
...
if (rawKeyboardState[currentKeyNumber]== 1 && keyboard[currentKeyNumber].state == 0)
if (keyboard[currentKeyNumber].debounceCount > 5)
keyboard[currentKeyNumber].state = 1;
else keyboard[currentKeyNumber].debounceCount++;


if (rawKeyboardState[currentKeyNumber] == 0 && keyboard[currentKeyNumber].state == 1)
if (keyboard[currentKeyNumber].debounceCount > 5)
keyboard[currentKeyNumber].state = 0;
else keyboard[currentKeyNumber].debounceCount++;
...
if(!(lastINTransmission_k1==0?0:((volatile BDT_ENTRY*)lastINTransmission_k1)->STAT.UOWN))
{

# 223
for (key = 0; key < 65; key++)
{
if (keyboard[key].state == 1 && keysDetected < 6)
{
...
hid_report_in_k1[0] = modifier;
hid_report_in_k1[1] = 0;
hid_report_in_k1[2] = keyArray[0];
hid_report_in_k1[3] = keyArray[1];
hid_report_in_k1[4] = keyArray[2];
hid_report_in_k1[5] = keyArray[3];
hid_report_in_k1[6] = keyArray[4];
hid_report_in_k1[7] = keyArray[5];


lastINTransmission_k1 = USBTransferOnePacket(3,1,(BYTE*)hid_report_in_k1,0x08);
}
}
